    ...
    docker.image('bagaspm12/submission-react-app').inside('-p 3000:3000') {
        ...
        stage('Deploy') {
            ...
            //    sh 'ssh -i $PRIVATE -o StrictHostKeyChecking=no $USER@54.179.63.68 docker stop submission-react-app'
                sh 'ssh -i $PRIVATE -o StrictHostKeyChecking=no $USER@54.179.63.68 docker run --rm -d --name submission-react-app -p 3000:3000 bagaspm12/submission-react-app bash ./jenkins/scripts/deliver.sh'
                }
            }
        }
    }